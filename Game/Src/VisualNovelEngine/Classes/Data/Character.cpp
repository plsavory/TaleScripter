...
Character::Character(int cId, std::string cFirstName, std::string cSurname, std::string cBio, std::string cAge,
                     bool cShowOnCharacterMenu, DatabaseConnection *novelDb) {
    id = cId;
    ...
    for (int i = 0; i < spriteCount; i++) {
        ...
        int characterId = std::stoi(dataSet->getRow(i)->getColumn("character_id")->getRawData());
        characterSprite[i] = new CharacterSprite(name, textureId, characterId);
    }
...
Character::Character(DataSetRow *data) {
    throw GeneralException("This is currently unimplemented.");
}
